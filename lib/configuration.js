...
var Configuration = (function () {
    ...
    Configuration.prototype.projects = function () {
        var _this = this;
        if (this._projectsFilled) {
            return this._projects;
        }
        _.each(this.content, function (v, key) {
            if (key === "defaults") {
                return;
            }
            _this._projects[key] = Configuration._makeProject(_this.content[key]);
            if (_.isString(_this._projects[key].path)) {
                _this._projects[key].rpath = u.resolveExpandEnv(_this._projects[key].path);
            }
        });
    ...
    Configuration.prototype.statFor = function (pKey) {
        this.statContent = this.statContent || {};
        this.statContent[pKey] = this.statContent[pKey] || {};
        return this.statContent[pKey];
    };
    Configuration.prototype.fetchGet = function (pKey) {
        var stat = this.statFor(pKey);
        return stat['fetch'] || 0;
    };
    Configuration.prototype.fetchIncrement = function (pKey) {
