...
var App = (function () {
    ...
    App.prototype.main = function (argv) {
        ...
        confLookup.forEach(function (f) {
            if (conf !== null) {
                return;
            }
            var parsed = Configuration.open(u.resolveExpandEnv(f));
            if (parsed !== null) {
                conf = _this.configuration = parsed;
        ...
        if (program['listProjects']) {
            ...
            _(conf.projects()).each(function (p, key) {
                console.log(util.format(" - %s (%s)", key, util.inspect(conf.content[key], false, undefined, true)));
            });
            return;
        }
        ...
        var projectsTasks = _(conf.projects()).map(function (pConf, pKey) {
            return _this.handleProject(pKey, pConf, projectMode, context, pretend);
        }).value();
        // Running all projects, in series
        async.series(projectsTasks, function () {
            conf.saveStat();
    ...
    App.prototype.handleProject = function (pKey, pConf, projectMode, context, pretend) {
        var _this = this;
        ...
        if (!p.check()) {
            o.warning(util.format("Project '%s' is now invalid, please check your configuration.", pKey), o.indent(2));
            return null;
        }
        ...
        return function (callback) {
            ...
            if (!pretend) {
                ...
                if (pConf['force_gc']) {
                    ...
                    if (_this.configuration.fetchGet(pKey) % pConf['gc_interval'] === 0) {
            ...
            async.series(tasks, function (err, res) {
                ...
                if (_.isObject(res['fetch']) && res['fetch']['code'] === 0) {
                    _this.configuration.fetchIncrement(pKey);
                }
                callback(null, res);
            });
        };
    };
    App.prototype.addProject = function () {
        var _this = this;
        var projectDir = u.resolveExpandEnv(this.program['addProject']), projectName = path.basename(projectDir);
    ...
    App.prototype.editConfig = function () {
    ...
    App.prototype.setOption = function (path, value) {
    ...
    App.prototype.unsetOption = function () {
        this.setOption(this.program['unsetOption'], undefined);
    };
    App.prototype.fail = function (code) {
        if (!_.isNumber(code)) {
            code = 1;
        }
        if (readline_on_fail) {
